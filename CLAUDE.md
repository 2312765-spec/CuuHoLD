...
**Bổ sung 2026-09-18 — zoom hết cỡ vẫn mất nền + toast, cả với `.fr/hot`.** Nhận định "`.fr` trả tile rừng hợp lệ ở z18" ở trên là **SAI** (chỉ thử 1 điểm). Đo lại nhiều điểm: `.fr/hot` trả **404 bền vững** (thử lại vẫn 404, không phải đang vẽ) ở vùng thưa dữ liệu — rừng (107.9, 11.75) từ z16, Phú Quý từ z17, Phan Thiết ở z18; chỉ trung tâm Đà Lạt đủ tới z18. Trang 404 của `.fr` CÓ kèm CORS `*`. Sửa: `utils/tileLayer.ts` → class `LopTileNen`, tile là `<div>` bọc `<img>`; tile lỗi thì lùi về tile cha (tối đa 4 mức) và phóng to đúng phần tương ứng — chỉ báo `tileerror` (→ toast) khi lùi hết vẫn lỗi. URL tile cha ghép bằng `L.Util.template(TILE_URL)` vì `getTileUrl()` gốc lấy z từ zoom hiện tại của map, bỏ qua `coords.z`. Test `tileLayer.spec.ts` (đỏ → xanh); ảnh chụp Chrome headless z18: rừng có nền (mờ, phóng từ z15), Đà Lạt sắc nét, 0 tileerror. Bài học: đánh giá nguồn tile phải thử **nhiều vùng**, không chỉ 1 điểm. ⚠️ Lúc kiểm chứng, agent đặt profile Chrome headless trong `frontend/` → watcher Vite gặp `EBUSY` và **làm sập `npm run dev`** — luôn để profile/ảnh tạm ngoài thư mục dự án.
